    ...
    try:  # Trapping a OSError or FileNotFoundError:  File permissions problem I believe
        for entry in os.scandir(path):
            if entry.is_file() and my_regex_obj.search(entry.path):  #
                bools = [True for txt in contains_txt if txt in entry.path and (exclude_text == '' or exclude_text not in entry.path)]
                if len(bools) == len(contains_txt):
                    yield entry.stat().st_size, entry.stat().st_atime_ns, entry.stat().st_mtime_ns, entry.stat().st_ctime_ns, os.path.normpath(entry.path)
